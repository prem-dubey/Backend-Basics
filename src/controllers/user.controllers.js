...
const getUserChannelProfile = asyncHandler(async (req , res)=>{ //add verifyJWT to load user
    const {username} = req.params //suppose it's in the url 
    if (!username?.trim()) {
        throw new ApiError(400,'username not found through params')
    }
    //we get arrays after aggregation
